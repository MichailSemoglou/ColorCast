...
def visualize_transfer_result(
    ...
    if show_histograms:
        ...
        for i, (color, name) in enumerate(zip(colors, channel_names, strict=False)):
            ax_hist = subplot(start_row + i, 0, colspan=3)

            for img, label in [(source, "Source"), (reference, "Reference"), (result, "Result")]:
...
def visualize_method_comparison(
    ...
    n_image_rows = 2 if show_difference else 1
    n_rows = n_image_rows + (3 if show_histograms else 0)

    if figsize is None:
        figsize = (5 * (n_methods + 1), 5 * n_image_rows + 3 * (3 if show_histograms else 0))
    ...
    if show_histograms:
        ...
        for ch_idx, (ch_color, ch_name) in enumerate(zip(colors, channel_names, strict=False)):
            row = n_image_rows + ch_idx

            hist, bins = np.histogram(reference[:, :, ch_idx], bins=50, range=(0, 1))
            axes[row, 0].plot(bins[:-1], hist, color=ch_color, alpha=0.7, linewidth=1.5)
            axes[row, 0].set_title(f"Reference {ch_name}", fontsize=9)
            axes[row, 0].set_xlabel("Intensity", fontsize=8)
            axes[row, 0].set_ylabel("Count", fontsize=8)
            axes[row, 0].grid(True, alpha=0.3)

            for i, (name, img) in enumerate(images.items(), start=1):
                hist, bins = np.histogram(img[:, :, ch_idx], bins=50, range=(0, 1))
                axes[row, i].plot(bins[:-1], hist, color=ch_color, alpha=0.7, linewidth=1.5)
                axes[row, i].set_title(f"{name} {ch_name}", fontsize=9)
                axes[row, i].set_xlabel("Intensity", fontsize=8)
                axes[row, i].set_ylabel("Count", fontsize=8)
