...
class MethodComparison:
    ...
    @staticmethod
    def compute_histogram_distance(img1: np.ndarray, img2: np.ndarray, bins: int = 256) -> float:
        """
    ...
    def _make_metrics(
        ...
        """
        Build the standard four-metric dictionary for a method result.

        Args:
            result: Transferred image array (H, W, 3), or None when the
                transfer method raised an error.
            source_img: Source image array (H, W, 3) in range [0, 1].
            reference_img: Reference image array (H, W, 3) in range [0, 1].
            error_msg: Human-readable error string stored under ``_error``
                when ``result`` is None.

        Returns:
            Dictionary with four numeric quality metrics:
            ``psnr``, ``ssim``, ``color_distance``, and
            ``histogram_distance``.  When ``result`` is None every metric
            is NaN; if ``error_msg`` was supplied an ``_error`` key is
            also included.
        """
        if result is None:
            ...
            if error_msg is not None:
                metrics["_error"] = error_msg
            return metrics
        return {
            "psnr": self.compute_psnr(result, reference_img),
            "ssim": self.compute_ssim(result, reference_img),
            "color_distance": self.compute_color_distance(source_img, result),
            "histogram_distance": self.compute_histogram_distance(result, reference_img),
        }
    ...
    def compare_methods(
        ...
        if include_baseline:
            results["baseline"] = self._make_metrics(source, source, reference)
        ...
        for name, method in methods.items():
            try:
                ...
                Exception
            ) as exc:  # noqa: BLE001 — isolate per-method failures so one bad method cannot abort the comparison
                logger.error("Comparison method %r failed: %s", name, exc, exc_info=True)
                results[name] = self._make_metrics(None, source, reference, error_msg=str(exc))
                continue

            results[name] = self._make_metrics(result, source, reference)
    ...
    def rank_methods(
        ...
        return sorted(
            [
                ...
                if name != "baseline"
                and not np.isnan(metrics.get(primary_metric, float("nan")))
                and not np.isclose(metrics.get("color_distance", 1.0), 0.0, atol=1e-9)
            ],
            key=lambda x: x[1],
    ...
    def find_best_method(
        ...
        ``psnr``, ``ssim``, and ``histogram_distance`` measure fidelity to
        the reference image (higher PSNR/SSIM is better; lower histogram
        distance is better).  ``color_distance`` measures deviation from the
        source image (lower is better) — an identity method that returns
        the source unchanged is therefore excluded from ranking.
        ...
        if not ranking:
            raise ValueError(
                f"No valid results for metric {metric!r} — " "all methods failed or returned NaN."
            )
        return ranking[0]
